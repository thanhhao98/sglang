...
Startup enforces containment: server-args validation and `initialize_model_parallel` both reject `attn_tp_size % dcp_size != 0`, and each rank asserts post-init that its DCP group rank matches the `attn_tp_rank % dcp_size` derivation.
