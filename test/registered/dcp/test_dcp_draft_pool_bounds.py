"""Test DCP-aware EAGLE draft KV-pool budgeting.

The draft pool spans the allocator's widened loc space (replicated, not
sharded), so the EAGLE draft term in cell_size must scale with
get_parallel().attn_dcp_size — the same rule applied to the DFLASH term.
"""
...
class TestEagleDraftPoolDCPBudget(CustomTestCase):
    ...
    def test_no_invented_draft_depth(self):
        """A None draft depth skips the scaling; it must not get a guessed floor."""
        from sglang.srt.model_executor.pool_configurator import _eagle_draft_layers
