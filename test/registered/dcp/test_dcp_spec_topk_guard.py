"""Test _validate_dcp_spec: reject tree drafting (topk > 1) under DCP.

The DCP verify path folds draft tokens as a linear causal chain, so EAGLE /
EAGLE3 / DFLASH require topk == 1. DSPARK is not gated: it never resolves
speculative_eagle_topk and already ships with DCP.
"""
