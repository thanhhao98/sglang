"""Test draft_forward_guard: the DCP-disable scope for draft forwards.

The draft KV pool is replicated, not sharded, so a draft forward must see
dcp_enabled == False (attn_dcp_size == 1, attn_dcp_rank == 0). The guard is
the only mechanism enforcing this since ModelRunner.dcp_size was removed.
"""
