...
class TestTpaCommunicator(unittest.TestCase):
    def _context(self, *, attention_outputs_are_tpa_replicated: bool = False):
        return CommunicateContext(
            ...
            tp_size=2,
            tp_rank=0,
            attention_outputs_are_tpa_replicated=attention_outputs_are_tpa_replicated,
        )

    def _layer_communicator(
        self,
        *,
        ...
        if use_full_tp_attention_handoff:
            layer_modes.middle_residual_mode = ScatterMode.FULL
        server_args = types.SimpleNamespace(
            speculative_algorithm=None,
            is_attention_tpa_enabled=lambda: attention_outputs_are_tpa_replicated,
        )
        with mock.patch(
            ...
            return LayerCommunicator(
                ...
                use_full_tp_attention_handoff=use_full_tp_attention_handoff,
            )
        ...
        with mock.patch(
            ...
            out, out_residual = (
                CommunicateWithAllReduceAndLayerNormFn._gather_hidden_states_and_residual(
                    ...
                    forward_batch=None,
                    layernorm=layernorm,
                    context=self._context(attention_outputs_are_tpa_replicated=True),
        ...
        with mock.patch(
            ...
            out, out_residual = (
                CommunicateWithAllReduceAndLayerNormFn._gather_hidden_states_and_residual(
                    ...
                    forward_batch=None,
                    layernorm=layernorm,
                    context=self._context(attention_outputs_are_tpa_replicated=False),
                    residual_input_mode=ScatterMode.TP_ATTN_FULL,
                )
            )
    ...
    def test_tpa_postprocess_disables_reduce_scatter_shortcut(self):
        layer = self._layer_communicator(attention_outputs_are_tpa_replicated=True)
    ...
    def test_full_tp_handoff_skips_postprocess_scatter(self):
        layer = self._layer_communicator(use_full_tp_attention_handoff=True)
        forward_batch = types.SimpleNamespace(
            dp_padding_mode=types.SimpleNamespace(is_max_len=lambda: True)
        )
        server_args = types.SimpleNamespace(
        ...
        with mock.patch(
            ...
            "sglang.srt.layers.communicator.dp_reduce_scatter_tensor",
            side_effect=AssertionError(
                "full-TP handoff should not trigger reduce-scatter in postprocess"
            ),
        ), mock.patch(
            "sglang.srt.layers.communicator.dp_scatter",
            side_effect=AssertionError(
                "full-TP handoff should not trigger scatter in postprocess"
            ),
        ):
            hidden_states, residual = layer.postprocess_layer(
                torch.randn(2, 8), torch.randn(2, 8), forward_batch
            )

        self.assertEqual(hidden_states.shape, (2, 8))
        self.assertEqual(residual.shape, (2, 8))

    def test_full_tp_handoff_uses_full_tp_reduce_path(self):
        hidden_states = torch.randn(2, 8)
        residual = torch.randn(2, 8)
        layernorm = _FakeLayerNorm()
        server_args = types.SimpleNamespace(
        ...
        with mock.patch(
            ...
            "sglang.srt.layers.communicator.attn_tp_all_reduce",
            side_effect=AssertionError(
                "attention TP reduce should not be used after full-TP handoff"
            ),
        ), mock.patch(
            "sglang.srt.layers.communicator.tensor_model_parallel_all_reduce",
        ...
        self.assertEqual(tp_reduce.call_count, 1)
        self.assertTrue(torch.equal(out, hidden_states + 4))
        self.assertTrue(torch.equal(out_residual, residual))
