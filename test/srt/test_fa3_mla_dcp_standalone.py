...
Usage (inside Docker container with GPU):
    python3 test/srt/test_fa3_mla_dcp_standalone.py
"""
...
def run_fa3_mla_decode(
    q_rope, qv, k_rope_cache, v_cache, page_table, cache_seqlens, softmax_scale=None,
):
    """Run FA3 MLA decode and return (output [B,1,H,D_v], lse [B,H,1])."""
...
def build_kv_cache(total_tokens, num_kv_heads, d_rope, d_nope, page_size, device):
    """Build a synthetic paged KV cache."""
    num_pages = (total_tokens + page_size - 1) // page_size
    k_rope = torch.randn(num_pages, page_size, num_kv_heads, d_rope, device=device, dtype=torch.bfloat16)
    v_nope = torch.randn(num_pages, page_size, num_kv_heads, d_nope, device=device, dtype=torch.bfloat16)
    return k_rope, v_nope, num_pages


def build_page_table_identity(B, seq_len, page_size, device):
    """Page table where page i maps to page i (identity mapping)."""
...
def shard_kv_for_dcp(k_rope_full, v_nope_full, seq_len, page_size, dcp_rank, dcp_size, B, device):
    ...
    Returns: (k_rope_local, v_nope_local, local_page_table, local_seqlens)
    """
    ...
    k_local = torch.zeros(B * local_pages_per_seq, page_size, num_kv_heads, d_rope, device=device, dtype=torch.bfloat16)
    v_local = torch.zeros(B * local_pages_per_seq, page_size, num_kv_heads, d_nope, device=device, dtype=torch.bfloat16)

    local_page_table = torch.zeros(B, local_pages_per_seq, dtype=torch.int32, device=device)
...
class TestFA3MLASingleGPU(unittest.TestCase):
    """Step 1: FA3 MLA on single GPU — ground truth."""
    ...
    def test_basic_mla_output(self):
        """FA3 MLA decode produces valid output and LSE."""
        torch.manual_seed(42)
        B, H_q, H_kv, d_rope, d_nope = 2, 8, 1, 64, 512
        seq_len, page_size = 32, 16

        q_rope = torch.randn(B, 1, H_q, d_rope, device=self.device, dtype=torch.bfloat16)
        qv = torch.randn(B, 1, H_q, d_nope, device=self.device, dtype=torch.bfloat16)

        k_cache, v_cache, _ = build_kv_cache(B * seq_len, H_kv, d_rope, d_nope, page_size, self.device)
        ...
        out, lse = run_fa3_mla_decode(q_rope, qv, k_cache, v_cache, page_table, cache_seqlens)
    ...
    def test_deterministic(self):
        ...
        out1, _ = run_fa3_mla_decode(q_rope, qv, k_cache, v_cache, page_table, cache_seqlens)
        out2, _ = run_fa3_mla_decode(q_rope, qv, k_cache, v_cache, page_table, cache_seqlens)

        torch.testing.assert_close(out1, out2, atol=0, rtol=0)


class TestFA3MLASimulatedDCP(unittest.TestCase):
    """Step 2: Simulate DCP sharding, combine partials, compare to ground truth."""
    ...
    def _run_dcp_simulation(self, B, H_q, H_kv, d_rope, d_nope, seq_len, page_size, dcp_size):
        """Run full attention and simulated DCP, return both results."""
        ...
        k_full, v_full, _ = build_kv_cache(total_tokens, H_kv, d_rope, d_nope, page_size, self.device)
        ...
        out_full, _ = run_fa3_mla_decode(q_rope, qv, k_full, v_full, pt_full, seqlens_full, softmax_scale=scale)
        ...
        for rank in range(dcp_size):
            ...
            out_r, lse_r = run_fa3_mla_decode(
                q_rope, qv, k_local, v_local, pt_local, seqlens_local, softmax_scale=scale,
            )
            # out_r: [B, 1, H_q, d_nope], lse_r: [B, H_q, 1]
            partial_outputs.append(out_r.squeeze(1))  # [B, H_q, d_nope]
            partial_lses.append(lse_r.squeeze(-1))     # [B, H_q]

        # Stack: [N, B, H_q, d_nope] and [N, B, H_q]
        ...
        # combined: [B, H_q, d_nope]

        return out_full.squeeze(1), combined  # both [B, H_q, d_nope]

    def test_dcp2_matches_full(self):
        """DCP=2 simulated sharding + combine must match full attention."""
        out_full, out_dcp = self._run_dcp_simulation(
            B=2, H_q=8, H_kv=1, d_rope=64, d_nope=512, seq_len=32, page_size=16, dcp_size=2
        )
        torch.testing.assert_close(out_full.float(), out_dcp.float(), atol=0.05, rtol=0.02)

    def test_dcp4_matches_full(self):
        """DCP=4 simulated sharding + combine must match full attention."""
        out_full, out_dcp = self._run_dcp_simulation(
            B=2, H_q=16, H_kv=1, d_rope=64, d_nope=512, seq_len=64, page_size=16, dcp_size=4
        )
        torch.testing.assert_close(out_full.float(), out_dcp.float(), atol=0.05, rtol=0.02)

    def test_dcp8_matches_full(self):
        """DCP=8 simulated sharding + combine must match full attention."""
        out_full, out_dcp = self._run_dcp_simulation(
            B=1, H_q=8, H_kv=1, d_rope=64, d_nope=512, seq_len=128, page_size=16, dcp_size=8
        )
        torch.testing.assert_close(out_full.float(), out_dcp.float(), atol=0.05, rtol=0.02)

    def test_dcp2_large_seq(self):
        """DCP=2 with longer sequence."""
        out_full, out_dcp = self._run_dcp_simulation(
            B=4, H_q=8, H_kv=1, d_rope=64, d_nope=512, seq_len=256, page_size=16, dcp_size=2
        )
        torch.testing.assert_close(out_full.float(), out_dcp.float(), atol=0.05, rtol=0.02)
