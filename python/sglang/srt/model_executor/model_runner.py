...
class ModelRunner(ModelRunnerKVCacheMixin):
    ...
    def _flashinfer_autotune(self):
        ...
        #
        # NOTE: we intentionally keep ``enable_symm_mem = True`` during autotune.
        # The autotune dummy run exercises every symm-mem code path (TP,
        # DCP, MoE) at realistic shapes, which warms up each group's NCCL
        # mempool with a variety of allocation sizes. Disabling symm-mem
        # here was observed to break DCP configs (cfg5/cfg6 hung at the
        # first prefill) because the DCP mempool stayed empty until the
        # first live request and then raced on ncclCommWindowRegister.
    ...
    def forward_extend(
        ...
        if not skip_attn_backend_init:
            if get_dcp_world_size() > 1:
                ...
                extend_cu_prefix_lens[1:] = torch.cumsum(
                ...
                def create_dcp_kv_indices(
                    ...
                    prefix_start = (
                        tl.load(extend_cu_prefix_lens_ptr + pid) // dcp_world_size
                    )
                ...
                forward_batch.dcp_kv_indices = dcp_kv_indices
                forward_batch.dcp_extend_prefix_lens_sum = extend_prefix_lens_sum
    ...
    def prealloc_symmetric_memory_pool(self):
        ...
        # The tensor is allowed to go out of scope so its chunk is returned
        # to the TP symm-mem mempool's free list, where it can be reused by
        # smaller NCCL allocations during serving (avoiding fragmentation).
        # PyTorch's caching mempool keeps the chunk reserved without calling
        # ncclMemFree, so no collective free is triggered at process exit.
        #
        # NOTE: we only prealloc for the TP group. Other symm-mem groups
        # (DCP, MoE TP, etc.) are warmed up by the FlashInfer autotune dummy
        # run (_flashinfer_autotune), which exercises each group's code path
        # at realistic shapes while symm-mem is still enabled.
        with torch.get_device_module(self.device).stream(self.forward_stream):
            logger.info(
