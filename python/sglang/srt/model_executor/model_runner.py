...
class ModelRunner(ModelRunnerKVCacheMixin):
    ...
    def forward_extend(
        ...
        if not skip_attn_backend_init:
            ...
            if get_dcp_world_size() > 1 and self.use_mla_backend:
                # Only MLA extend paths currently materialize a batch-local DCP KV
                # staging view. Standard MHA + FlashAttention paths read KV from the
                # normal pool and use DCP metadata built inside the backend.
