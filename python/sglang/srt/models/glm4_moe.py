...
class Glm4MoeSparseMoeBlock(nn.Module):
    def __init__(
        ...
        # GLM-4.7 v6 (L16): keep use_grouped_topk=True (same path v3 used) — the
        # actual L16 win comes from a short-circuit inside biased_grouped_topk_impl
        # (topk.py) that skips the no-op grouping when num_expert_group=1.
        # Earlier attempts to flip use_grouped_topk to False triggered TorchDynamo
        # fake-tensor-dispatch recursion during piecewise CUDA graph compilation
        # (py-spy stack: stuck in compile_inner → __torch_dispatch__ nested);
        # keeping the same dispatch path avoids that issue entirely.
        self.topk = TopK(
            top_k=self.top_k + self.num_fused_shared_experts,
            layer_id=self.layer_id,
