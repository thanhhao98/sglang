...
def biased_grouped_topk_impl(
    ...
    # GLM-4.7 v6 (L16): when num_expert_group == 1 (or topk_group is None / 1),
    # the grouping operations below are mathematical no-ops:
    #   - group_scores has shape [n, 1] (one group per token)
    #   - group_idx is always [0] for every token (only group)
    #   - group_mask becomes all-1, score_mask becomes all-1
    #   - tmp_scores == scores_for_choice (no masking effect)
    # Skipping them eliminates 2 of 3 gatherTopK calls per MoE layer
    # (5,073 → ~1,691 in deep profile), saving ~240 ms / 7.8% busy time at
    # TP=8 cc=128. Output is bit-equivalent for n_group=1 configs (GLM-4.7).
