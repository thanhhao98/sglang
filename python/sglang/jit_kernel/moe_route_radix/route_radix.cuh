...
// radix narrowing. One CTA per token; all 256 threads cooperate (parallel histogram,
...
__global__ void route_radix_kernel(const __nv_bfloat16* __restrict__ scores,
                                   const float* __restrict__ bias,
                                   float* __restrict__ out_w, int32_t* __restrict__ out_i,
                                   int M, long long stride_sm, long long stride_wm,
                                   long long stride_im, float routed_scaling_factor,
                                   int renormalize, int apply_scale) {
    ...
    for (int shift = 24; shift >= 0; shift -= 8) {
        ...
        for (int n = t; n < N; n += kThreads) {
            ...
            if ((k & hi_mask) == (pref & hi_mask))
                atomicAdd(&s_hist[(k >> shift) & 0xFFu], 1);
        }
        __syncthreads();
    ...
    if (t == 0) {
        float sum = 0.0f;
        for (int a = 0; a < K; ++a) sum += s_sact[a];
