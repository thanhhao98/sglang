...
FOLDER_PATH=bench_result_news
BASE_OUTPUT="${FOLDER_PATH}/${BRANCH}_${HASH}"
...
# ---- Config definitions ----
# Format: CONFIG_NAME|BACKEND|MEM_FRAC|DCP|DCP_COMM|Q_REP
CONFIGS=(
   "tp8_flashinfer|flashinfer|0.85|0||0"
   "tp8_fa3|fa3|0.85|0||0"
   "tp8_dcp8_agrs_flashinfer|flashinfer|0.85|8|ag_rs|0"
   "tp8_dcp8_agrs_fa3|fa3|0.83|8|ag_rs|0"
   "tp8_dcp8_a2a_flashinfer|flashinfer|0.85|8|a2a|0"
   "tp8_dcp8_a2a_fa3|fa3|0.83|8|a2a|0"
#    "tp8_dcp8_a2a_flashinfer_q_rep|flashinfer|0.85|8|a2a|1"
#     "tp8_dcp8_a2a_fa3_q_rep|fa3|0.83|8|a2a|1"
)
...
wait_for_server() {
    ...
    while [ $elapsed -lt $max_wait ]; do
        local status=$(curl -s -o /dev/null -w "%{http_code}" "http://${HOST}:${PORT}/health" 2>/dev/null)
        if [ "$status" = "200" ]; then
...
run_perf() {
    local output_dir="$1"
    echo "Running performance benchmarks -> ${output_dir}"
...
start_server() {
    ...
    if [ "$q_rep" -eq 1 ]; then
        extra_args="${extra_args} --dcp-replicate-q-proj"
    fi

    echo "======================================================="
    echo "Starting server: ${cfg_name}"
    echo "  backend=${backend}  mem=${mem_frac}  dcp=${dcp}  comm=${dcp_comm}  q_rep=${q_rep}"
    echo "======================================================="

    eval "${env_prefix} python3 -m sglang.launch_server ${COMMON_ARGS} \
...
for cfg in "${CONFIGS[@]}"; do
    IFS='|' read -r CFG_NAME BACKEND MEM_FRAC DCP DCP_COMM Q_REP <<< "$cfg"
    ...
    start_server "$CFG_NAME" "$BACKEND" "$MEM_FRAC" "$DCP" "$DCP_COMM" "$Q_REP"
