#!/bin/bash
###############################################################################
# Full C3 DCP8 A2A FlashInfer benchmarks on H100 (optimized code)
# Runs all 4 context lengths × 10 concurrency levels = 40 points
###############################################################################
set -u
...
start_a2a_server() {
    ...
    python3 -m sglang.launch_server \
        ...
        --host $HOST --port $PORT \
        > /output/server_c3_opt_${CTX}.log 2>&1 &

    wait_for_server
}

echo "========================================="
echo "C3 DCP8 A2A FlashInfer - Optimized Code"
echo "========================================="

# --- 128K (mem_frac=0.50, same as baseline) ---
start_a2a_server 131072 0.50
for C in "${CONCURRENCIES[@]}"; do
    run_bench "C3_opt_128K" $C 130048
done

# --- 256K ---
start_a2a_server 262144 0.80
for C in "${CONCURRENCIES[@]}"; do
    run_bench "C3_opt_256K" $C 261120
done

# --- 512K ---
start_a2a_server 524288 0.75
for C in "${CONCURRENCIES[@]}"; do
    run_bench "C3_opt_512K" $C 523264
done

# --- 1M ---
start_a2a_server 1048576 0.65
for C in "${CONCURRENCIES[@]}"; do
    run_bench "C3_opt_1M" $C 1047552
done
...
echo "Results in /output/C3_opt_*.log"
echo "========================================="
