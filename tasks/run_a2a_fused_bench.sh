#!/bin/bash
###############################################################################
# Full C3-fused DCP8 A2A FlashInfer benchmarks (fused all_to_all)
# + GSM8K accuracy test
###############################################################################
set -u
...
start_server() {
    ...
    echo "Starting server (ctx=$CTX, mem_frac=$MEM_FRAC)..."
    python3 -m sglang.launch_server \
        ...
        --host $HOST --port $PORT \
        > /output/server_fused_${TAG}_${CTX}.log 2>&1 &
    wait_for_server
}

# Mem fracs: H100 128K needs 0.50, rest use standard
if [ "$TAG" = "h100" ]; then
    MF_128K=0.50
else
    MF_128K=0.85
...
echo "================================================"
echo "${TAG}: C3-fused DCP8 A2A FlashInfer"
echo "================================================"

# --- Accuracy (128K server) ---
...
python3 /sgl-workspace/sglang/benchmark/gsm8k/bench_sglang.py \
    ...
    --host $BENCH_HOST --port $PORT \
    2>&1 | tee /output/C3_fused_${TAG}_gsm8k.log

# --- 128K perf ---
for C in "${CONCURRENCIES[@]}"; do
    run_bench "C3_fused_${TAG}_128K" $C 130048
done

# --- 256K ---
start_server 262144 0.80
for C in "${CONCURRENCIES[@]}"; do
    run_bench "C3_fused_${TAG}_256K" $C 261120
done

# --- 512K ---
start_server 524288 0.75
for C in "${CONCURRENCIES[@]}"; do
    run_bench "C3_fused_${TAG}_512K" $C 523264
done

# --- 1M ---
start_server 1048576 0.65
for C in "${CONCURRENCIES[@]}"; do
    run_bench "C3_fused_${TAG}_1M" $C 1047552
done

kill_server
echo ""
echo "================================================"
echo "${TAG} fused benchmarks complete!"
echo "================================================"
